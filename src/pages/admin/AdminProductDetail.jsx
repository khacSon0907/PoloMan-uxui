...
function getStock(product) {
  return getColors(product).reduce((sum, color) => {
    return sum + getSizes(color).reduce((sizeSum, size) => sizeSum + Number(size?.quantity || 0), 0)
  }, 0)
}

function getImageCount(product) {
  return getColors(product).reduce((sum, color) => sum + getImages(color, product).length, 0)
}
      ...
      .catch(async (error) => {
        try {
          ...
          const foundProduct = Array.isArray(list)
            ? list.find((item) => String(getProductId(item)) === String(id) || item?.slug === id)
            : null
        ...
        if (isMounted) {
          setErrorMessage(getApiMessage(error, 'Khong the tai chi tiet san pham.'))
        }
      })
      .finally(() => {
        if (isMounted) {
  ...
  if (errorMessage || !product) {
    return (
      <section className="rounded-lg border border-neutral-200 bg-white p-6 text-center">
        <h2 className="text-lg font-semibold text-neutral-950">Khong tim thay san pham</h2>
        ...
        <div className="grid gap-5 p-4 sm:p-5 lg:grid-cols-[360px_1fr]">
          <div className="space-y-4">
            <div className="overflow-hidden rounded-lg border border-neutral-200 bg-neutral-100">
              {mainImageUrl ? (
                <img src={mainImageUrl} alt="" className="aspect-square w-full object-cover" />
              ) : (
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-md border border-neutral-200 p-3">
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-400">Mau</p>
                <p className="mt-2 text-2xl font-black text-neutral-950">{colors.length}</p>
              </div>
              <div className="rounded-md border border-neutral-200 p-3">
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-400">Anh</p>
                <p className="mt-2 text-2xl font-black text-neutral-950">{getImageCount(product)}</p>
              </div>
              <div className="rounded-md border border-neutral-200 p-3">
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-400">Ton</p>
                <p className="mt-2 text-2xl font-black text-neutral-950">{getStock(product)}</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-5">
            <section className="rounded-lg border border-neutral-200 p-4">
              <h3 className="text-base font-semibold text-neutral-950">Thong tin</h3>
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.14em] text-neutral-400">Gia</p>
                  <p className="mt-1 text-xl font-black text-neutral-950">
                    {formatCurrency(product.salePrice || product.price)}
                  </p>
                  {product.salePrice && (
                    <p className="mt-1 text-sm text-neutral-400 line-through">{formatCurrency(product.price)}</p>
                  )}
                </div>
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.14em] text-neutral-400">Trang thai</p>
                  <span
                ...
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.14em] text-neutral-400">Danh muc</p>
                  <p className="mt-1 text-sm font-semibold text-neutral-950">
                    {product.category?.name || product.categoryName || product.categoryId || '-'}
                  </p>
                </div>
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.14em] text-neutral-400">ID</p>
                  <p className="mt-1 break-all text-sm font-semibold text-neutral-950">{getProductId(product)}</p>
                </div>
              </div>
              {product.description && (
            ...
            <section className="space-y-4">
              {colors.map((color, colorIndex) => {
                ...
                return (
                  <div key={`${color.colorName}-${colorIndex}`} className="rounded-lg border border-neutral-200 p-4">
                    <div className="flex flex-col gap-3 border-b border-neutral-100 pb-3 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-neutral-500">{getSizes(color).length} size</p>
                      </div>
                      {color.colorCode && (
                      ...
                      <div>
                        ...
                        <div className="overflow-hidden rounded-md border border-neutral-200 bg-neutral-100">
                          {getImageUrl(mainImage) ? (
                            <img src={getImageUrl(mainImage)} alt="" className="aspect-square w-full object-cover" />
                          ) : (
                            <div className="flex aspect-square items-center justify-center text-sm font-semibold text-neutral-400">
                              No image
                            </div>
                        ...
                        {subImages.length ? (
                          <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 xl:grid-cols-6">
                            {subImages.map((image, imageIndex) => (
                              ...
                              >
                                <img src={getImageUrl(image)} alt="" className="aspect-square w-full object-cover" />
                              </div>
                            ))}
                          </div>
                        ) : (
                        ...
                        <tbody className="divide-y divide-neutral-100">
                          {getSizes(color).map((size, sizeIndex) => (
                            <tr key={`${size.size}-${sizeIndex}`}>
                              <td className="px-3 py-2 text-sm font-semibold text-neutral-950">{size.size}</td>
                              <td className="px-3 py-2 text-sm text-neutral-500">{size.sku || '-'}</td>
