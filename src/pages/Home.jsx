...
import { formatCurrency, getProductId, getProductImage, getProductSlug, productApi } from '../features/product'
...
function Home() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([categoryApi.list(), bannerApi.listActive(), productApi.getAll()]).then(
      ([categoryResult, bannerResult, productResult]) => {
        if (!isMounted) return
  ...
  return (
    <div className="space-y-12 pb-8 sm:space-y-16 sm:pb-10 lg:space-y-20 lg:pb-12">
      ...
      <section className="mx-auto grid max-w-7xl grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 lg:gap-6">
        {[
          { icon: 'SHIP', title: 'Mien phi van chuyen', desc: 'Don hang tu 499.000d toan quoc' },
          { icon: '30D', title: 'Doi tra de dang', desc: 'Ho tro doi tra trong vong 30 ngay' },
          { icon: 'QC', title: 'Chat lieu cao cap', desc: 'Kiem dinh chat luong chat che' },
          { icon: '24/7', title: 'Ho tro tan tam', desc: 'Cham soc khach hang moi ngay' },
        ].map((item) => (
          <div
            key={item.title}
          ...
          >
            ...
            <div>
              ...
              <p className="mt-0.5 text-[11px] text-emerald-900/60">{item.desc}</p>
            </div>
          </div>
        ))}
      </section>
      ...
      {visibleCategories.length > 0 && (
        <section className="space-y-6">
          <div className="space-y-2 text-center">
            <h2 className="text-2xl font-black uppercase tracking-widest text-neutral-900">Danh muc noi bat</h2>
            <p className="text-xs text-neutral-400">Thiet ke toi gian, ton vinh phong thai nam</p>
          </div>
      ...
      <section className="space-y-6">
        <div className="flex flex-col items-center justify-between gap-4 border-b border-emerald-100 pb-4 sm:flex-row">
          <div className="space-y-1 text-center sm:text-left">
            <h2 className="text-2xl font-black uppercase tracking-widest text-emerald-950">San pham moi</h2>
            <p className="text-xs text-emerald-900/55">Du lieu san pham duoc lay truc tiep tu admin</p>
          </div>
          <Link
            to="/products"
        ...
        {featuredProducts.length > 0 ? (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 lg:gap-6">
            {featuredProducts.map((prod, index) => {
              ...
              return (
                ...
                >
                  <Link to={`/products/${getProductSlug(prod)}`} className="relative block aspect-[4/5] overflow-hidden bg-emerald-50 sm:h-64 sm:aspect-auto">
                    {imageUrl ? (
                      <img
                        src={imageUrl}
                        alt={prod.name}
          ...
          <div className="rounded-lg border border-dashed border-neutral-200 bg-neutral-50 p-10 text-center">
            <h3 className="text-sm font-bold text-neutral-900">Chua co san pham hien thi</h3>
            <p className="mt-2 text-sm text-neutral-500">Hay tao san pham active trong trang admin.</p>
          </div>
        )}
      </section>
    </div>
